...
export class PreparationNode implements OnChanges, OnInit {
  ...
  create() {
    ...
    const r = Math.tan(Math.PI / 6) * (this.size.height / 2);
    this.path = [
      ['M', r, NODE_PADDING],
      ['L', this.size.width - r, NODE_PADDING],
      ['L', this.size.width - 2 * NODE_PADDING, this.size.height / 2],
      ['L', this.size.width - r, this.size.height - 2 * NODE_PADDING],
      ['L', r, this.size.height - 2 * NODE_PADDING],
