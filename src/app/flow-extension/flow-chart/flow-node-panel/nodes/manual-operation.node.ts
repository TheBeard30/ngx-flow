...
export class ManualOperationNode implements OnChanges, OnInit {
  ...
  create() {
    this.rx = Math.min(Math.tan(Math.PI / 6) * this.size.height, this.size.width / 2);
    this.path = [
        ['M', NODE_PADDING, NODE_PADDING],
        ['L', this.size.width - 2 * NODE_PADDING, NODE_PADDING],
        ['L', this.size.width - this.rx, this.size.height - 2 * NODE_PADDING],
        ['L', this.rx, this.size.height - 2 * NODE_PADDING],
        ['Z'],
    ];
