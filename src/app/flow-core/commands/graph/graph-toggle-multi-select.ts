  ...
  execute = async () => {
    ...
    const result = await hooks.toggleMultiSelect.call(
      ...
      async handlerArgs => {
        ...
        const graphEnableMultiSelectModel = await MODELS.GRAPH_ENABLE_MULTI_SELECT.getModel(modelService);
        const needEnableRubberBand: boolean = typeof isEnable === 'boolean' ? isEnable : !x6Graph.isRubberbandEnabled();
